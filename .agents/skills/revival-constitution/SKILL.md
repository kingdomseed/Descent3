...
- Keep RevivalEditor, D3Import, and dedicated no-window hosting macOS-only. Use concrete AppKit and UIKit shells over the same RevivalCore, RevivalMetal, canonical world, scheduler, renderer, and package paths; do not add a generic platform, input, filesystem, renderer, or simulation abstraction. UIKit owns RevivalMobile's application lifecycle and is its default UI toolkit. Permit a bounded SwiftUI view only when a current component has a recorded concrete advantage and remains hosted inside the same UIKit lifecycle, navigation, state, and Metal view path.
...
- Let the macOS/shared lane establish each source-led contract through Phase 7. Early RevivalMobile target, build, and simulator composition is useful compatibility evidence, not a separate closure checkpoint. Unavailable mobile hardware, signing, or development-team setup cannot make a Phase 1–7 packet or phase `HOLD`. Phase 8 owns required physical mobile integration, with applicable multiplayer and release evidence in Phases 9 and 10. This ordering never permits a temporary Mac-only Core, renderer, package schema, scheduler, input model, or lifetime path and never removes RevivalMobile or touch from version 1.0.
...
4. Identify the concrete limitation. For performance or lifetime claims, capture an optimized representative M4 measurement for the affected Mac/shared path when its owning checkpoint requires one. Beginning with Phase 8, a claim that includes mobile behavior uses the applicable physical iPhone and iPad evidence. Before Phase 8, unavailable mobile hardware, signing, or development-team setup is a deferred integration fact rather than a blocker.
