...
11. Run the affected suite and applicable complete-Level, Mac-player, editor-isolation, failure-preservation, and local-retail evidence. Through Phase 7, early mobile target automation or simulator intake may prove only the package wiring it executes; unavailable devices, signing, or development-team setup cannot hold the import packet open. Phase 8 runs the production file-picker-to-app-storage path on physical devices for the package schema, promotion, handoff, and consumer behavior it exercises. No mobile path ever exposes a retail source to the target.
...
- the promoted canonical package validates and the consumers applicable to the claimed checkpoint prove no retail format is opened; a parser-only checkpoint closes through production D3Import, a Phase 1–7 Mac/shared integration checkpoint adds RevivalMac and RevivalEditor, early mobile compatibility evidence proves only what it executes, and Phase 8 proves physical file-picker intake, copy into app-owned storage, and shared native-package validation;
...
A parser-only or Phase 1–7 Mac/shared import packet does not wait on mobile hardware, signing, or development-team setup. It also does not claim Phase 8 physical mobile intake or version 1.0 readiness. Do not report a synthetic parser pass as Training import, a package write as atomic-promotion proof, a simulator-only picker exercise as physical mobile package-intake proof, or a build as import correctness.
