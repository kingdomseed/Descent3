...
description: Implement, review, and verify the one direct Swift and MSL Metal 4 renderer, complete-Level presentation lifetime, source-faithful visibility, and final-GPU-use release for RevivalMac and RevivalEditor.
---

# Metal 4 rendering

Build the smallest real Metal 4 path that transfers the current Descent 3 visual contract into both native applications. Preserve source-evidenced results and lifetime; do not preserve a legacy graphics API or prebuild a modern engine framework.
...
The current direction is Apple Silicon, macOS 26 or later, direct Swift 6.4 and MSL, and Metal 4 from Xcode 27. Use the current beta until stable Xcode 27 replaces it. A conflicting skill, sample, or available API does not change that baseline.
...
Phase 1 loads one complete canonical `Level` and renders one selected room as its acceptance view. The selected room never becomes a partial production `Level`, room package, alternate loader, or editor-only renderer.

## Smallest real frame path

Use one concrete RevivalMetal implementation shared by separately owned player, editor-document, and editor-play-session values:

1. AppKit and `MTKView` own the window, drawable, and presentation callback.
...
- profile an optimized build on the recorded M4 with validation and capture overhead disabled, recording CPU extraction, encoding, GPU time, allocation, and unified-memory evidence.
