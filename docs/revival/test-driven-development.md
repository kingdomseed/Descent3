...
Manual play, Metal captures, Instruments traces, and accessibility review supplement automation where they observe something automation cannot. Beginning with the Phase 8 mobile integration gate, physical iPhone and iPad checks cover orientation and safe areas, UIKit lifecycle, audio routes and interruptions, memory warnings, thermal behavior, touch/physical-controller handoff, and later local-network permission. Simulator evidence cannot replace those device effects, but unavailable devices, signing, or development-team setup cannot make a Phase 1–7 packet `HOLD`. Manual evidence does not replace the first failing automated test for project-owned behavior. When an operating-system effect cannot be automated reliably, the red test covers the nearest project-owned decision or command and the manual evidence covers the final integration at its owning phase.
