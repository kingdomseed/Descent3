...
- six-degree-of-freedom flight including afterburner boost with fuel, recharge and its separately acquired cooler, ship wiggle/bob and its explicit authoritative-or-presentation decision, autoleveling and mouselook control modes, level gravity, environmental and impulse forces, damaging rooms and volatile/lava/water surfaces, the player headlight, and the declared exponential drag integration; five named difficulty levels and their declared AI, weapon, energy-and-shield-pickup, AI energy-drop, generic-damage, profile, session, and behavior-query rules, collision, objects, doors, triggers, AI including source-supported thief behavior, purpose-built volumetric navigation, separately ledgered ordinary and advanced weapon mechanics, resource and weapon pickups, cloak, invulnerability, quad fire, RapidFire, converter, countermeasure inventory and named countermeasure behaviors, GuideBot and its command menu, matcens, goals, terrain, weather, effects, damage, death and the death camera, refueling rooms, and progression;
- briefings, movies, runtime camera-path cinematics with player-control and presentation transitions, independently testable TelCom goal, ship-status, and ship-selection surfaces, HUD element modes, animated three-dimensional cockpit, three-dimensional automap and discovery state, player markers and messages, rear view and auxiliary camera views for GuideBot, guided weapons and markers, UI/point/object sound, portal-aware spatial sound, ambient patterns, room reverb, terrain-altitude ambience, in-level streamed mission voice, adaptive music, post-level results screens, and the application shell and its menu, new/load/replay/multiplayer, level, failure, completion, pause/focus/resume, credits, intro, and loading transitions; the exact source-supported localization matrix, pilot-profile lifecycle, separately owned input/gameplay, display/detail, and audio/voice/haptic preferences, player save-slot/quicksave workflow with failure-safe storage, screenshots, campaign level-warp, controller support, native haptics, operational diagnostics, and the intended cheat, easter-egg, and diagnostic command surface with per-command availability and consequences; mobile touch input, scene suspension, safe-area and changing drawable behavior, audio-session interruptions and routes, canonical-package intake, app-owned storage, and physical-device resource pressure; accessibility and localization improvements are adopted explicitly as their slices arrive rather than assumed as an undefined 1.0 umbrella;
...
- physics, animation, AI, death, effect, and object-archetype definitions, plus source-supported inventory operations through behavior authoring rather than an invented general inventory-definition tool;
...
- cinematic sequencing, behavior-message catalogs and reference-safe renaming, string management, the exact source-supported locale and fallback matrix, localization preview, and validation;
...
- native modern-font authoring with glyph coverage, metrics, kerning, atlas generation and localization preview, plus independently testable animation-state, death, effect, ambient-audio, reverb, adaptive-score and sound-event bindings and inventory behavior operations.
...
- package publishing, provenance, signing where required, and distribution, followed by distinct RevivalMac and RevivalMobile intake surfaces over one shared local-library contract that validates, stages, installs, activates, disables, replaces or removes native packages without a marketplace, cloud account, or hosted service;
...
Complete revival status requires the shared player rows to pass in both RevivalMac and RevivalMobile, the platform-specific rows to pass in their owning shell, and all ledger rows to be either:
