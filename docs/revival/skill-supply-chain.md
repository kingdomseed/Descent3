...
| OthmanAdi [`planning-with-files`](https://github.com/OthmanAdi/planning-with-files) | audited local checkout `f90780c92f0506d21c5f6c4865ce95517a8b1964`; MIT, copyright Ahmad Adi; reviewed July 16, 2026 | restore durable context before acting, re-read state before decisions, write back after a phase, and leave a resumable handoff | Adapted in [`revival-documentation-steward`](../../.agents/skills/revival-documentation-steward/SKILL.md) using original project prose and the existing accepted documents and ledgers; no external files were imported and no scripts or hooks were executed. Excludes `task_plan.md`, `findings.md`, `progress.md`, `.planning/`, per-agent ledgers, fixed read/write quotas, error diaries, prompt injection, recitation, attestation, loops, automated stop-gate hooks, templates, scripts, and parallel plans |
...
| User-supplied `Cold Review Arbiter Skill` draft | local design artifact supplied July 23, 2026; no reusable-text license or canonical repository supplied | treat findings as claims, classify the governing contract, steelman contrary evidence, accept only the supported kernel, and record explicit rejection dispositions | Principles only, rewritten as original Revival-specific rules inside [`revival-review`](../../.agents/skills/revival-review/SKILL.md); no text, scripts, standing reviewer hierarchy, Mythic repository ownership, Flutter, Bloc, Freezed, ADR, VGV, or framework-specific assumptions imported |
...
## Project-authored completion review

[`revival-completion-gate`](../../.agents/skills/revival-completion-gate/SKILL.md) is original project prose created after the Slice 3 closure process promoted later-milestone profiling, unsupported failure events, repeated verification, and unlimited negative proof into self-created blockers. It derives its rules from the accepted engineering, testing, verification, review, and checkpoint contracts. It imports no external text, scripts, dependencies, hooks, templates, or automated decision mechanism. It runs once as a bounded read-only senior review after ordinary concern consolidation and cannot waive an accepted current-checkpoint requirement or concrete reachable defect.

## Evaluated and deferred sources

These local checkouts were audited on July 16, 2026 and are not installed project skills. Reconsider only at the named evidence boundary; do not copy their payloads or run their tooling merely because the checkout exists.

| Source | Pin and license | Potential value | Current disposition |
| --- | --- | --- | --- |
| AvdLee [`Swift-Concurrency-Agent-Skill`](https://github.com/AvdLee/Swift-Concurrency-Agent-Skill) | `0d472de78225d2875283c35eaca1c060c493bdb3`; MIT | inspect language mode, strict-concurrency settings, default isolation, and the actual isolation boundary before interpreting a diagnostic | At the first real asynchronous import or editor boundary, adapt only that settings-first diagnostic into [`swift-realtime-systems`](../../.agents/skills/swift-realtime-systems/SKILL.md) if current source and compiler evidence still justify it. Do not import its generic actor, task-group, or task-spawning defaults. |
| AvdLee [`Swift-Testing-Agent-Skill`](https://github.com/AvdLee/Swift-Testing-Agent-Skill) | `798e9b1a2bcac164d4f0c781908199e754f0bab6`; MIT | Swift Testing API mechanics for expectations, requirements, parameterization, and XCTest boundaries | Defer until a real test target exposes an API-level gap. Any adaptation must be checked against Xcode 27 and the binding TDD contract, replace its seconds-based time-limit example with the [supported minute-granularity API](https://developer.apple.com/documentation/testing/timelimittrait/duration), and exclude test-only protocols, fake repositories, disabled or quarantined protecting tests, and broad known-issue use. |
| AvdLee [`Xcode-Build-Optimization-Agent-Skill`](https://github.com/AvdLee/Xcode-Build-Optimization-Agent-Skill) | `6bd7b596cd688b1127ded00e812b1b6937ec35d6`; MIT | benchmark-first, wall-clock build analysis with an approval gate | Defer until a real workspace has measured build pain. Before any use, audit every Python helper, dependency, network or telemetry path, artifact write, and mutating recommendation; do not create its parallel optimization-plan workflow now. |
| Charles Wiltgen [`Axiom`](https://github.com/CharlesWiltgen/Axiom) | `8eec4b97f491635b0735c8d805efe06e9d9c9e8e`; MIT | current Apple-platform and Swift 6.4 reference material | Reject wholesale installation: its broad mandatory triggers duplicate the project skill system, and the checkout contains executable hooks, shell, JavaScript, Python, MCP, and binary-execution tooling. A later domain may audit one exact Markdown reference at the pinned revision when a concrete gap exists. |
