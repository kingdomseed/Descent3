...
The schema-11 Guidebot chain may also independently omit or bind the exact stock `Yellow flare` page, `FlareYellowBright.OOF` and its four reached carrier materials, the first reached `yellowspark.oaf` frame and `Flare` → `Flare.wav` canonical PCM. The weapon/page reader preserves version-8 physics and lighting field order; the bounded OAF reader validates its header, positive frame count/time and every contained OGF frame before selecting the first reached particle frame. D3Import remains the only table, OOF/OAF and WAV reader. Older schema-11 packages remain valid and silent; a present partial or duplicate-name binding fails before simulation. Runtime, retained Metal and disposable editor play consume only the canonical model/material/sound values and never reopen retail containers. This is not schema 12, a weapon catalog, generic particle system or second content path.
