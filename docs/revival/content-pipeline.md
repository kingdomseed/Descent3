...
The package is an ordinary directory package using deterministic Codable JSON for structured values and ordinary Apple-readable files for media where practical. It declares one project-owned content type so the same directory package can cross the macOS and mobile system document-picker boundary; this is packaging metadata, not a second archive or schema. Schema 11 is the current behavior/presentation-usable boundary. It retains schema 3's canonical selected-view RGBA, typed material/procedural, reached lightmap requirements, object-presentation references and canonical OOF model hierarchy, geometry, material, submodel-presentation, provenance and LOD values; schema 4 adds complete resident surface physics; schema 5 adds model collision radii, the reached ship definition and default-player binding; schema 6 makes force-field texture state canonical; schemas 7–9 add the opening lesson, gallery barrier and robot/Guidebot chains with their exact voices, models, navigation and continuation state. Schema 10 adds the exact Camera Monitor/SecurityCamera identities, pickup radius/messages/timers, GPNT-derived camera position/forward, `GuideBotC.osf` and `GuideBotD.osf`, and `PupC1` → `PupC.wav` canonical PCM/provenance. Schema 11 adds the exact Script 043–046 `RASBot1`–`RASBot4` death identities and reused `gyro.OOF` presentations; Script 047's `InvulnPowerup2` handle 2076 / room 12 identity, `Invulnerability` model page and `Powerup pickup` → `Power03.wav`, `Invon.wav` and `Invoff.wav`; Script 048's `CloakPowerup2` handle 2073 / room 11 identity, `cloak.OOF` / `CloakMed.OOF` / `CloakLow.OOF` presentation, `ShpCloakOn.wav` / `ShpCloakOffBeep.wav`, plus the direct Script 034/049 PortalRoom6, FlashLight-4 and timer-tail contract; Script 050's exact Portal4 room-44/face-1 trigger, final-room messages and `Intro7.osf` entry/format/frame/source/decoded-PCM identity while reusing that barrier and marker contract; and Scripts 051–055's exact `LastBot1` handle 4127 / room 14, `LastBot2` handle 2080 / room 46, `LastBot3` handle 2081 / room 47, `LastBot4` handle 2082 / room 47 and `LastBot5` handle 2083 / room 48 dying-death identities while reusing the existing `RAS1 Light Security Flyer`, `gyro.OOF` presentation and stock Level 2 Blue combat value. This is a smallest forward extension of the unreleased schema rather than schema 12, a behavior catalog or a legacy script carrier. D3Import remains the only OSF/ACM, PCM16-WAV, OOF/page and D3L reader. Its bounded ACM decoder preserves the released one-zero-byte final refill rule; the reached sound path resolves only the selected retail sound pages and validates PCM16 WAV once at import. Reached WAV and OOF readers fail closed on checked PCM byte arithmetic, exact RIFF body/chunk padding and the expected OOF magic/version before accepting payloads. Its OOF boundary reads the reached GPNT parent-offset chain for SecurityCamera without turning the runtime into a model parser, alongside the existing `$facing`, `$rotate=` and `RANI` support. Core receives typed lesson/barrier/combat/Guidebot/Camera Monitor/RASBot/pickup/timed-effect/navigation/model/audio values and validates exact stock identity/digests; AppKit forms transient WAVE containers for AVFoundation. No player/editor legacy reader, runtime sound/page/model reader, second navigation representation or second package format is added. The shared validator accepts a landed schema-9 predecessor whose absent `soundClips` member decodes as the schema-9 empty default, while schemas 10 and 11 require that member and the Camera Monitor chain; earlier unsupported packages remain rejected.

Schema 11 optionally binds exact `GBExpulsionA.wav` canonical PCM and provenance to the concrete Training Guidebot chain for command-0 release. Older schema-11 packages omit that field and release silently; a present binding without its clip and dependency fails validation. This does not create schema 12, an authored sound catalog or a runtime legacy reader.

The same schema-11 chain may independently omit or bind exact `GBotEngineB1` → `GBotEngineB.wav` canonical PCM for the reached post-birth ambient engine loop. D3Import validates the stock page's full-file loop, spatial-update flags, 10/100 distance range, volume 0.1 and exact media fingerprint before emitting the concrete clip/dependency. Older schema-11 packages remain valid and silent; a present binding without the admitted clip fails validation. Runtime and editor play consume the canonical clip through the existing direct AppKit owner, using the already-canonical reciprocal room portals, path points and door position for its reached non-adjacent spatial result; they never reopen the table or WAV container.

The schema-11 Guidebot chain may also independently omit or bind the exact stock `Yellow flare` page, `FlareYellowBright.OOF` and its four reached carrier materials, all six `yellowspark.oaf` frames at source frame time `0.07`, and `Flare` → `Flare.wav` canonical PCM. The weapon/page reader preserves version-8 physics and lighting field order; the bounded OAF reader validates its header, exact six-frame count, exact `0.07` source time and every contained OGF frame before admitting the concrete animation value. D3Import remains the only table, OOF/OAF and WAV reader. Older schema-11 packages with only the first image remain readable through a static first-frame fallback; a new partial, misordered or duplicate-name animation binding fails before simulation. Runtime, retained Metal and disposable editor play consume only the canonical model/material/sound values and never reopen retail containers. This is not schema 12, a weapon catalog, generic animation or particle system, or second content path.

The reached schema-11 ship definition may independently omit or bind one exact stock player-flare value: Pyro-GL battery 20 with mask bit 0, `Yellow flare`, logical `Flare`, wait 1, zero energy/ammo/fire/battery flags, and model `GPNT0` parent 0 at local position `(0.0000004444,-1.0462444,3.1798251)` with local forward `(0.0000076296,-0.0000152587,1)`. Older schema-11 packages omit the nested value and physical G remains silent; a partial or mismatched present value fails at the canonical boundary. Core, retained Metal and direct Mac/editor AppKit play consume that typed binding through the existing Yellow path. This adds neither schema 12 nor a runtime ship-page/model reader, alternate simulation owner or authored weapon surface.

Schema 11 also optionally carries the later maneuver-and-follow extension without a version bump: exact `ManuverRoomCenter`/`FollowBot1` identities and transforms, `gyro.OOF` presentation, the 13-node `FollowLoop1` and one-node `GoToDie` paths, source movement values, exact messages and five voice provenance records. D3Import remains the only reader of the retail object/page/path/message/audio forms. Older schema-11 packages omit the extension; Core and RevivalEditor consume only the canonical typed result, and the stock paths remain read-only rather than becoming a second creator/navigation format.
