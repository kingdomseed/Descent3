...
The package is an ordinary directory package using deterministic Codable JSON for structured values and ordinary Apple-readable files for media where practical. It declares one project-owned content type so the same directory package can cross the macOS and mobile system document-picker boundary; this is packaging metadata, not a second archive or schema. Schema 9 is the current behavior/presentation-usable boundary. It retains schema 3's canonical selected-view RGBA, typed material/procedural, reached lightmap requirements, object-presentation references and canonical OOF model hierarchy, geometry, material, submodel-presentation, provenance and LOD values; schema 4 adds complete resident surface-physics coverage; schema 5 adds exact model collision radii, the reached typed ship definition and the canonical default-player binding required by the active player/editor presentation; schema 6 makes the already imported force-field texture flag a canonical surface behavior; schema 7 adds the first typed Training opening lesson and two validated canonical PCM voice clips with exact prepared-installation HOG entry, archive, source SHA and PCM SHA provenance plus matching current dependency-manifest records. Schema 8 adds the exact one-shot `Portal2` / two-sided `PortalRoom4` force-field binding, exact marker handle/type/name and message state, plus canonical `GuideBotA.osf` source and PCM identity. Schema 9 adds the exact `DestroyBot2` identity, shields and stock laser contract, destruction continuation, deployed Guidebot definition and movement state, restored controls and control-help state, exact Script 036/060 messages, canonical `Buddybot.oof` and `gyro.OOF`, canonical `GuideBotB.osf` and `proceed5.osf` identities, and the released verified indoor `NODE` graph. The source-led existing buddy is represented by promoting retired D3L handle 6164 into a hidden canonical type-2 `GuideBotB` object with a hidden `Buddybot.oof` presentation; this makes its identity and retained draw payload part of the immutable canonical base before F4 activates it. D3Import remains the only OSF/ACM, OOF/page and D3L reader. Its bounded ACM decoder matches the released libacm input refill rule by supplying the single zero byte that completes a final filler and declared tail at physical EOF; any subsequent bit demand remains truncated. Its OOF boundary recognizes the reached `$facing` directive and the exact `$rotate=` rate plus `RANI` axis required by `gyro.OOF`, maps the reached `SecFlyLit-Flashers000` slot through the resolved `red flare` page, and retains rejection of unreached `$thruster` and malformed or axis-less rotation. Its D3L boundary normalizes the released boundary-edge cost to at least one while retaining verification and clearance values. Core receives typed lesson/barrier/combat/Guidebot/navigation/model-presentation/PCM values, validates each PCM digest and additionally binds the five exact stock PCM digests, while the AppKit presenter forms the transient WAVE container used by AVFoundation. No player/editor legacy reader, synthetic sound-page identity, runtime D3L reader or second navigation representation is added. The shared validator rejects older packages rather than silently accepting content that cannot satisfy the current player and editor contract.
