...
The package is an ordinary directory package using deterministic Codable JSON for structured values and ordinary Apple-readable files for media where practical. It declares one project-owned content type so the same directory package can cross the macOS and mobile system document-picker boundary; this is packaging metadata, not a second archive or schema. Schema 6 is the current presentation-usable boundary. It retains schema 3's canonical selected-view RGBA, typed material/procedural, reached lightmap requirements, object-presentation references and canonical OOF model hierarchy, geometry, material, submodel-presentation, provenance and LOD values; schema 4 adds complete resident surface-physics coverage; schema 5 adds exact model collision radii, the reached typed ship definition and the canonical default-player binding required by the active player/editor presentation; schema 6 makes the already imported force-field texture flag a canonical surface behavior so the shared simulation can apply the released default player response without reopening a legacy table at runtime. The shared validator rejects older topology or presentation packages rather than silently accepting content that cannot satisfy the current player and editor contract.
