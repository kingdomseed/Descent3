...
- Status: accepted research plan; evidence collection pending
- Date: July 15, 2026
...
LAN is already bounded: direct hosting and Bonjour discovery use Network framework. This study selects the Internet boundary. It creates no production transport types, schemas, targets, provider integration, or deployment until its evidence and decision are accepted.
...
- Apple platform-provided multiplayer services that are actually available to this macOS product and its required session shapes;
...
| Session fit | Source-supported mode counts, released 32 network/player-slot infrastructure, four-player campaign co-op, observers, and dedicated hosting |
...
| Integration | Current APIs, SDK or service dependencies, signing and entitlement constraints, deployment shape, and update burden |
...
1. evidence owner, investigation dates, sources, test environments, usage and cost assumptions;
...
The decision is acceptable only when at least one topology satisfies the required Internet outcome with a viable operator and cost boundary, every rejected candidate has evidence rather than preference, unsupported reachability is explicit, historical service ideas receive recorded dispositions, and the selected path leaves one production mechanism.
