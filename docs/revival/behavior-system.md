...
The next separately censed stock Guidebot island reaches only Yellow-flare firing and its first retained carrier, particle and light. Every due flare tick first persists `3 + rand()/32767` from that same simulation-owned stream, even when helper-goal occupancy or missing optional content prevents creation. Eligibility is the concrete released slot fact `goal 1 used && goals 2 and 3 unused`: active-goal helper slots clear 3 then 2, goal-1 arrival installs the return slot 2, the coupled post-greeting draw clears it, and return-to-ship retains only slot 1 until physical entry. A generic goal or AI model is not introduced. Exact Yellow creation attempts `Flare` at the live Guidebot before gunpoint `-1` creation and adds no spread draw. Released LIFO object allocation places this reached carrier below the already-running Guidebot slot, so the firing update consumes only the reset; on the next ascending object update, the first reached weapon frame consumes six `yellowspark` values in x/y/z/speed/size/life order and `OLF_FLICKER_SLIGHTLY` consumes the seventh after weapon control. The one stream therefore advances eight times across those two ordered updates. The live carrier uses the existing indoor movement/collision owner, parent-local `PF_STICK`, source lifetime cleanup and schema-7 continuation, while retained Metal consumes the same frame values. The descendant presentation-only noise split is deliberately not adopted because the accepted current result preserves released presentation/gameplay coupling.

The bounded continuation of that carrier now completes the first parent-timeout generation through visual quiescence. Parent weapon control emits any final due `yellowspark` before strict `lifeleft < 0` timeout creates canonical `FlarePuff` and attempts exact `YellowFlareSparks` children in center-then-eight-ring order. Released LIFO allocation plus ascending object iteration gives source attempts 0…8 slots `[17, 8, 41, 42, 43, 44, 45, 46, 47]`: every child except slot 8 receives same-frame control. Every later child frame still decrements life, runs due weapon control even when life became negative, then silently deletes below zero before physics/light; exact zero receives stock gravity/mass/drag motion and fixed light 6. Child flags `0x420` select no `TimeoutWeapon` explosion branch and physics flags `0x270080` include `PF_NO_COLLIDE`, so child timeout is inert and source-silent rather than a deferred `yellowspark` explosion. After all objects run, `FlarePuff` and old or same-frame-created parent/child particles decrement life, move when nonnegative and retain exact zero. Admitted particles consume only the existing stream's six x/y/z/speed/size/life draws; the direct first-generation capacity is 54 and capacity rejection consumes none. Canonical creation time drives all six `yellowspark.oaf` frames: child objects use `Int(GameTime / 0.07) % 6`, particles use `Int(6 * clampedNormalizedAge)`, and opacity remains full through half life before fading over the second half. Schema 7 admits only pristine, source-reachable full/slot-8/visual-only and terminal states, keeps the consumed latch true after quiescence and rejects inconsistent partial or animation state. Later periodic parents remain supported but receive no first-generation source slot and create no children. Retained Metal removes the generation after terminal restore through the same plan. Older parent-only and one-frame schema-11 packages remain silent/static fallbacks. Impact, scorch, damage, a general allocator and generic weapon/effect behavior remain later only when reached by a real producer.
