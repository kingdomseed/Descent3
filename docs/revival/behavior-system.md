...
The next independently censed Phase 5 island reuses the sole primary-fire and indoor-contact owner after Script 026. One fixed volley spends `0.15` energy no more often than every `0.25` seconds and visibly presents two exact Level 2 Blue bolts through retained object draws; each applies `7.5` damage to live `FollowBot1` until the strict-below-zero real dying event. Script 031 runs in released level-interval order and restores energy to exactly 50 whenever Script 021 has run and energy is below 50. FollowBot1's dying Script 037 starts timer 11 for exactly two seconds, and timer Script 027 preserves the released `GoodJob` → both moving teams → DestroyBot1 unghost → moving-target instruction → `kill1.osf` → existing `FollowLoop1` assignment order. DestroyBot1 then circles through the same authored-static-path movement and room-collision carrier with velocity-facing orientation. The direct continuation silently preserves every reached counter, timer, ghost/team value and motion state while admitting the source-valid timer-only Script-027 coincidence. DestroyBot1 damage/death and Script 028, the later DestroyBot2 chain and general moving-target combat remain later islands.
