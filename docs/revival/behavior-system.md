...
The current independently censed Phase 5 island reuses the sole primary-fire and indoor-contact owner only after Script 027 has made DestroyBot1 visible and moving. One fixed volley spends `0.15` energy no more often than every `0.25` seconds and visibly presents two exact Level 2 Blue bolts through retained object draws; each applies `7.5` damage to DestroyBot1 until the strict-below-zero real dying event. Its dying-only Script 028 preserves the exact predicate `counter028 < 1 && counter027 > 0` and exact order: unghost DestroyBot2, assign the existing FollowLoop1 path 0 with flags `0x801100`, goal `-1` and priority `3`, then increment counter 028 last. DestroyBot2 becomes visible and circles through the same authored-static-path movement and room-collision carrier but remains non-targetable. The direct continuation silently preserves DestroyBot1 shields/death, counter 028, DestroyBot2 visibility and its existing path motion while older schema-7 values hydrate the newly reached state without a version bump. DestroyBot2 damage/death, Script 036/timer 12/Script 029 and general moving-target combat remain later islands.
