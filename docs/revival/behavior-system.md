...
Phase 4 begins with the chronological Training opening chain: level start creates the one-second welcome timer, the timer presents the welcome/forward instruction and voice, and first ForwardGoal contact presents success/reverse instruction and voice while switching the allowed control from forward to reverse. Ordinary typed Swift functions and a small explicit state value inside RevivalCore are the canonical current runtime form, not predeclared disposable scaffolding. The source ledger retains the exact generated, custom, handwritten, initialization, persistence, collision and editor-play ranges. Door, trigger, robot, route, weapon and broader behavior families remain separate evidence-led islands.
...
Schema 7 carries one typed `TrainingOpeningLesson`, the stable ForwardGoal handle, exact English message bindings, and canonical PCM clips with HOG entry, archive and SHA provenance plus matching dependency-manifest records. D3Import alone reads the released `.msg` and OSF/ACM forms. RevivalCore owns the hidden-but-collidable ForwardGoal presentation state, level-start timer, swept nonblocking contact, one-shot state, enabled-control transition and Codable continuation. Independent collision and timer results survive a same-frame coincidence in source order. RevivalAppKit renders ordinary two-message or coincident four-message results in a dynamically sized virtual-4:3 region and uses one direct AVFoundation player in both RevivalMac and disposable RevivalEditor play. That single owner preserves the released `StreamPlay` replacement rule: when contact and timer feedback share a frame, all four HUD lines remain ordered while the later `Welcome.osf` replaces `Return1.osf` as the active voice.

The reached creator path uses the existing stable object handle, object placement/configuration, named `Move Object` undo, deterministic project save/reopen and disposable play/return. That is sufficient for the represented ForwardGoal binding; it does not preselect the permanent authored-behavior representation. The first island reaches no BNode, BOA or robot route, so no navigation service or data translation is activated.
