...
The bounded continuation of that carrier now reaches its released strict `lifeleft < 0` timeout. Weapon control emits any final due parent `yellowspark` first; timeout then creates the canonical `FlarePuff` explosion before attempting exact `YellowFlareSparks` children in center-then-eight-ring order. The name `FlareHit` is not a retail texture page and is rejected as a preflight premise rather than translated. Released LIFO allocation plus ascending object iteration gives source attempts 0…8 slots `[17, 8, 41, 42, 43, 44, 45, 46, 47]`: every child except slot 8 receives same-frame control. Child weapon 54 has no light-flicker flag, so its light stays exactly 6 and consumes no draw; the timeout update therefore advances the one authoritative stream by 54 particle draws and the first following normal child frame advances it by 6 more. Those reached child frames apply the stock gravity force through exact mass and drag while `PF_NO_COLLIDE` keeps collision outside this boundary. Schema 7 retains the explosion, complete nine-child set and coherent first particles without replay; partial or duplicate timeout state rejects. The schema-11 timeout binding is optional, so an older parent-only binding remains valid and expires silently. Retained Metal consumes these same bounded results through the existing plan. Child impact or timeout, scorch, damage, another periodic flare and generic weapon/effect behavior remain separate islands.
