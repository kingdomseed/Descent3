...
Phase 4 proceeds through separately censed chronological Training chains. The opening chain creates the welcome timer and handles first ForwardGoal contact. The gallery chain consumes the one-shot player-center `Portal2` pass-through, closes both sides of the `PortalRoom4` force field, presents `GoodJob` then `GBIntro`, and disables controls. The robot/Guidebot chain reaches exact primary-laser death, Script 036's barrier/timer result, F4 birth, same-update Script 060 and one concrete route/movement result. The next bounded chain collides with Camera Monitor handle 6167, completes the exact enabled `Locate the Camera Monitor` object item and goal status, holds its object-backed inventory identity, consumes one inventory-use request, removes that identity only after copying its typed effect, and presents a ten-second SecurityCamera gunpoint view plus the reached two-second completion no-op. Pickup and use feedback remain ordered voice/sound/HUD effects, and audio-start failure cannot suppress later DALLAS effects. Training contains no `RoomDoor`; these concrete chains do not claim the general door, combat, AI, inventory or auxiliary-view families. Ordinary typed Swift functions and small explicit state values inside RevivalCore remain the canonical current runtime form. The source ledger retains each finite generated, custom, handwritten, initialization, persistence, producer and editor-play census; later Training and broader families remain separate evidence-led islands.
