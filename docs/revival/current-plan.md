...
| Field | State |
| --- | --- |
| Active roadmap phase | Phase 1 macOS/shared product entry is active after the reviewed combined Slice 0+1 landing. Remaining Phase 0 inventory work continues independently and does not block the next Mac/shared packet. |
| Active integration slice | None. Combined Slice 0+1 is the latest landed baseline: the minimal workspace, D3Import seam, AppKit shells, checked HOG2 parser, and malformed-boundary contracts are implemented and verified for the claims made. Slice 2 is next but remains provisional until its owner completes the bounded entry preflight. |
| Active-packet preflight | Complete for Slice 1. The owner checked `cfile/hogfile.{h,cpp}`, the HOG ranges of `cfile/cfile.cpp`, `cfile/tests/cfile_tests.cpp`, `tools/HogMaker/HogFormat.{h,cpp}`, `tools/HogMaker/HogMaker.cpp`, `legacy/hogedit/HogEditDoc.cpp`, `legacy/hogedit/HogEditView.cpp`, `legacy/hogmaker/hog.cpp`, the tracked fixture bytes, target composition, and focused seam. Confirmed corrections were to account for the active CMake writer, source tests, creator workflow intent plus HogEdit's defective 68/72-byte binary path, and the obsolete duplicate; to delete the four evidence-only research artifacts at their accepted gate; and to state the native boundary explicitly. The direct parser accepts a structurally complete zero-entry archive and header-to-payload padding, preserves source table order and spelling, and rejects truncated headers/tables/payloads, table or offset arithmetic overflow, table overlap, unexplained trailing bytes, non-ASCII names, nonterminated 36-byte name fields, out-of-order folded names, and folded duplicates/collisions. No linked unknown remains that requires wayfinding. |
| Active implementation lanes | None assigned. The project can use one implementation lane or an optional second lane when another developer and a genuinely independent packet both exist; the current capacity map does not require two active developers. |
| Packet owner | None active. `Codex task 019f6ad9-5966-7f43-98f6-f5b8965a8f9d` owned the combined Slice 0+1 implementation and its internal review through handoff. Name the Slice 2 owner before its production work begins. |
| Integration owner | None active for the next packet. `Codex task 019f66e3-9777-7a82-83c4-ca01c025a4e8` performed the final merge-gate review, documentation reconciliation, verification, and user-authorized landing; Slice 2 records its own integration owner at entry. |
| Product code | The landed workspace contains D3Import, RevivalMac, RevivalEditor, and `D3ImportTests`; RevivalCore and RevivalMetal remain source-ownership groups rather than speculative targets. `Revival/Sources/D3Import/HOG2Archive.swift` implements the checked direct parser, and `HOG2ArchiveTests.swift` owns the tracked fixture and malformed-boundary proof. D3Import remains fail-closed because Slice 0+1 does not add a retail-processing CLI path. No MSL, canonical model, `NSDocument`, RevivalMobile target, or Slice 2 behavior is present. |
...
| Prerequisite skill gate | `revival-constitution` and `revival-source-translation` were reconciled with the accepted documents on July 16, 2026. At Slice 1 resumption, `revival-source-translation`, `d3-content-import`, and `revival-verification` were reviewed against the active reader, active writer, source tests, creator caller, obsolete duplicate, tracked fixture, and current native seam; no skill amendment is required for this parser contract. |
| Current local toolchain | Slice 0 commands select `/Applications/Xcode-27-Beta-3.app/Contents/Developer` per invocation through `DEVELOPER_DIR`; the global `/Applications/Xcode.app/Contents/Developer` selection remains unchanged. Verified July 16, 2026: Xcode 27.0 build `27A5218g`, Swift 6.4 (`swiftlang-6.4.0.25.4`), Metal `32023.918`, macOS 27.0 SDK, iOS 27.0 device and simulator SDKs, and arm64 macOS 26.5.2 build `25F84`. Product settings use Swift 6 language mode (`SWIFT_VERSION = 6.0`) and macOS 26.0 deployment; there is no Xcode 26 or Swift 6.3 compatibility lane. |
| Product start blocker | None at project level. The project-owned missing-contract red is resolved through the checked Slice 1 parser without an Xcode 26, Swift 6.3, legacy runtime, or compatibility lane. Slice 2 still requires its ordinary owner assignment and bounded preflight before production begins. |
...
| Independent review | Final combined-target verdict: all four implementation concerns are `CLEAN`. The final root merge-gate then found three documentation-continuity issues: exact run evidence in the wrong owner, the independently testable HOG2 capability hidden in an umbrella functional row, and a capacity map that implied two developers were always expected. All three were corrected. Independent post-fix capacity, evidence/ledger, and whole-packet reviews found the corrections clean; their shared stale pre-commit-state finding was resolved by this landed-state handoff. Final verification reran the 17-test suite, all six Debug/Release product builds, document checks, and `rtk git diff --check`. |
| Integration blocker | None for the landed Slice 0+1 result. No separate Slice 0 landing or Wave 0 rebase/join exists. Slice 2's unassigned owner and unrun preflight are entry conditions for that next packet, not retroactive blockers on this landing. |
| Next packets | Slice 2 complete Training topology, then Slice 3 first direct Metal Mac/editor result and document loop, then Slice 4 first mobile composition. Each begins with its own bounded preflight; the plan activates only the lanes that have real owners and independent work. |
...
Implementation result: the checked parser and 17-test boundary suite satisfy this exit for review. The source row is `translating`, not `native-running` or `verified`, because the deliberately fail-closed D3Import entry point does not yet exercise a complete shipping import path.
...
`Codex task 019f6ad9-5966-7f43-98f6-f5b8965a8f9d` owned the Slice 0 scaffold, Slice 1 parser work, shared Xcode workspace/project/target/scheme/resource metadata, affected documentation, and internal review through handoff. `Codex task 019f66e3-9777-7a82-83c4-ca01c025a4e8` performed the final merge-gate review, resolved the documentation findings, reran verification, and landed the combined packet under explicit user authorization. No Wave 0 join remains: there was no separately landed metadata seam to rebase onto, and the deliberate Slice 0 compile red was preserved until the same tree became green in Slice 1.
...
| Slice 2 before canonical `Level` types land | D3Import owns MN3/HOG selection, D3L translation, the canonical topology contract, package identity, and the source and functional rows it changes | Direct Metal/AppKit shell work may advance only where it does not invent or edit unfinished canonical types, package schema, D3Import code, shared project metadata, or the same ledger rows | The primary lane lands the canonical contract; any consumer rebases before using it |
| Slice 2-to-3 integration after canonical types land | D3Import continues reachable dependency capture and validated package production | RevivalMetal, RevivalMac, and RevivalEditor consume the landed types for room-3 rendering and the document loop | One complete imported `Level` and package contract feeds the single Mac/editor path; no adapter or alternate schema survives |
