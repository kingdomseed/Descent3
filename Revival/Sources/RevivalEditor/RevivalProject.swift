...
struct RevivalProject: Equatable, Sendable {
    ...
    var trainingReturnRightSourceDiagnostic: String? {
        guard let returnRight =
                level.trainingOpeningLesson?.returnRight,
              let goal = level.objects.first(where: {
                  $0.handle == returnRight.leftGoalObjectHandle
              }) else {
            return nil
        }
        return "TrainingMission.cpp Script 004 / \(goal.instanceName ?? "LeftGoal") handle \(goal.handle) / \(returnRight.voiceSourceName)"
    }
    var trainingReturnUpSourceDiagnostic: String? {
        guard let returnUp =
                level.trainingOpeningLesson?.returnUp,
              let goal = level.objects.first(where: {
    ...
    var trainingRepeatForwardGoalSourceDiagnostic: String? {
        guard let repeatForwardGoal =
        ...
        return "TrainingMission.cpp Script 008 / \(goal.instanceName ?? "ForwardGoal") handle \(goal.handle) / \(repeatForwardGoal.soundLogicalName)"
    }
    var trainingRepeatReturnLeftSourceDiagnostic: String? {
        guard let repeatReturnLeft =
        ...
        return "TrainingMission.cpp Script 009 / \(goal.instanceName ?? "StartGoal") handle \(goal.handle) / \(repeatReturnLeft.voiceSourceName)"
    }
    var trainingRepeatReturnRightSourceDiagnostic: String? {
        guard let repeatReturnRight =
        ...
        return "TrainingMission.cpp Script 010 / \(goal.instanceName ?? "LeftGoal") handle \(goal.handle) / \(repeatReturnRight.soundLogicalName)"
    }
    var trainingRepeatReturnUpSourceDiagnostic: String? {
        guard let repeatReturnUp =
        ...
        return "TrainingMission.cpp Script 011 / \(goal.instanceName ?? "StartGoal") handle \(goal.handle) / \(repeatReturnUp.voiceSourceName)"
    }
    var trainingRepeatReturnDownSourceDiagnostic: String? {
        guard let repeatReturnDown =
        ...
        return "TrainingMission.cpp Script 012 / \(goal.instanceName ?? "UpGoal") handle \(goal.handle) / \(repeatReturnDown.soundLogicalName)"
    }
    var trainingContinueToCourseSourceDiagnostic: String? {
        ...
        return "TrainingMission.cpp Script 013 / \(goal.instanceName ?? "StartGoal") handle \(goal.handle) / PortalRoom1 portals 0,1 / \(continueToCourse.voiceSourceName)"
    }
    var trainingStartCourseSourceDiagnostic: String? {
        ...
        return "TrainingMission.cpp Script 014 / \(target.instanceName ?? "StartCourse") handle \(target.handle) / PortalRoom1 portal \(startCourse.portalIndex) / Intro1.osf"
    }
    var trainingFinishCourseSourceDiagnostic: String? {
    ...
    var trainingLastBot3DeathSourceDiagnostic: String? {
        ...
        return "TrainingMission.cpp Script 053 / \(robot.instanceName ?? "LastBot3") death"
    }

    var trainingLastBot4DeathSourceDiagnostic: String? {
        ...
        return "TrainingMission.cpp Script 054 / \(robot.instanceName ?? "LastBot4") death"
    }

    var trainingLastBot5DeathSourceDiagnostic: String? {
        ...
        return "TrainingMission.cpp Script 055 / \(robot.instanceName ?? "LastBot5") death"
    }
    ...
    var trainingFinalBotsCompletionSourceDiagnostic: String? {
